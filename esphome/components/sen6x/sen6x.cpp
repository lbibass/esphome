...
// Command definitions (from Sensirion SEN6x datasheet)
// static const uint16_t SEN6X_CMD_AUTO_CLEANING_INTERVAL = 0x8004; doesn't exist in SEN6x
static const uint16_t SEN6X_CMD_GET_DATA_READY_STATUS = 0x0202;
static const uint16_t SEN60_CMD_GET_DATA_READY_STATUS = 0xE4B8;
// static const uint16_t SEN6X_CMD_GET_FIRMWARE_VERSION = 0xD100; no firmware versio command in SEN6x
...
// static const uint16_t SEN6X_CMD_READ_MEASUREMENT = 0x03C4; NO READ MEASUREMENT, NOW SPECIFIC TO EACH SENSOR TYPE
static const uint16_t SEN6X_CMD_READ_MEASUREMENT_SEN60 = 0x03C4;
...
// static const uint16_t SEN6X_CMD_START_MEASUREMENTS_RHT_ONLY = 0x0037; no RHT only command in SEN6x
static const uint16_t SEN6X_CMD_STOP_MEASUREMENTS = 0x0104;  // SEN6x uses 0x0104 for stopping measurements
...
static const int8_t SEN6X_INDEX_SCALE_FACTOR = 10;
static const int8_t SEN6X_MIN_INDEX_VALUE = 1 * SEN6X_INDEX_SCALE_FACTOR;
static const int16_t SEN6X_MAX_INDEX_VALUE = 500 * SEN6X_INDEX_SCALE_FACTOR;
  ...
  this->set_timeout(1000, [this]() {
    ...
    this->set_timeout(stop_measurement_delay, [this]() {
      ...
      if (product_name_ == "SEN60") {
        ...
        if (product_name_ == "SEN63C") {
          sen6x_type = SEN63C;
        } else {
          if (product_name_ == "SEN65") {
            sen6x_type = SEN65;
          } else {
            if (product_name_ == "SEN66") {
              sen6x_type = SEN66;
            } else {
              if (product_name_ == "SEN68") {
                sen6x_type = SEN68;
              }
            }
          }
      ...
      if (this->humidity_sensor_ && sen6x_type == SEN60) {
        ESP_LOGE(TAG, "For Relative humidity a SEN63C, SEN65, SEN66, or SEN68 is required. You are using a <%s> sensor",
                 this->product_name_.c_str());
        this->humidity_sensor_ = nullptr;  // mark as not used
      }
      if (this->temperature_sensor_ && sen6x_type == SEN60) {
        ESP_LOGE(TAG, "For Temperature a SEN63C, SEN65, SEN66, or SEN68 is required. You are using a <%s> sensor",
                 this->product_name_.c_str());
        this->temperature_sensor_ = nullptr;  // mark as not used
      }
      if (this->voc_sensor_ && sen6x_type == SEN60 or sen6x_type == SEN63C) {
        ESP_LOGE(TAG, "For VOC a SEN65, SEN66, or SEN68 is required. You are using a <%s> sensor",
                 this->product_name_.c_str());
        this->voc_sensor_ = nullptr;  // mark as not used
      }
      if (this->nox_sensor_ && sen6x_type != SEN66 or sen6x_type != SEN68 or sen6x_type != SEN65) {
        ESP_LOGE(TAG, "For NOx a SEN63C, SEN66, or SEN68 is required. You are using a <%s> sensor",
                 this->product_name_.c_str());
        this->nox_sensor_ = nullptr;  // mark as not used
      }
      if (this->co2_sensor_ && sen6x_type == SEN60 or sen6x_type == SEN65) {
        ESP_LOGE(TAG, "For CO2 a SEN65 or SEN66 is required. You are using a <%s> sensor", this->product_name_.c_str());
        this->nox_sensor_ = nullptr;  // mark as not used
      }
      // there is no firmware version command in SEN6x.
      ...
      auto cmd = SEN6X_CMD_START_MEASUREMENTS;  // no RHT only command in SEN6x
      if (sen6x_type == SEN60) {
        cmd = SEN6X_CMD_READ_MEASUREMENT_SEN60;
      } else if (sen6x_type == SEN63C) {
        cmd = SEN6X_CMD_READ_MEASUREMENT_SEN63C;
      } else if (sen6x_type == SEN65) {
        cmd = SEN6X_CMD_READ_MEASUREMENT_SEN65;
      } else if (sen6x_type == SEN66) {
        cmd = SEN6X_CMD_READ_MEASUREMENT_SEN66;
      } else if (sen6x_type == SEN68) {
        cmd = SEN6X_CMD_READ_MEASUREMENT_SEN68;
      }
...
void SEN6XComponent::update() {
  ...
  if (!this->write_command(SEN6X_CMD_READ_MEASUREMENT_SEN60 or SEN6X_CMD_READ_MEASUREMENT_SEN63C or
                           SEN6X_CMD_READ_MEASUREMENT_SEN65 or SEN6X_CMD_READ_MEASUREMENT_SEN66 or
                           SEN6X_CMD_READ_MEASUREMENT_SEN68)) {
  ...
  this->set_timeout(20, [this]() {
    ...
    float co2 = measurements[8] == UINT16_MAX ? NAN : measurements[8];
